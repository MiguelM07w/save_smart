...
import bg from '../../assets/images/bg.png'; // <- misma convención que tu ejemplo
import './styles/UserProfile.css';

// Íconos inline tipo Apple
...
const UserProfile: React.FC = () => {
  ...
  // Estilo de fondo: solo la imagen aquí; los demás props en el CSS
  ...
  // Mutación simple - solo guarda cuando se presiona el botón
  ...
  // Subida de foto => NO guarda automáticamente, solo cambia el preview
  ...
  // Cambios locales; NO guarda automáticamente
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    if (name === 'email') return; // email no editable
  ...
  return (
    <div className="user-profile profile-bg" style={containerStyle}>
      <style>{`
        ...
        <div className="header-content">
          <div className="profile-avatar">
            {photoPreview ? (
              ...
              <span>{(formData.username || authUser?.username || 'U').charAt(0).toUpperCase()}</span>
            )}
          </div>
          <div>
            <h1>Mi Perfil</h1>
